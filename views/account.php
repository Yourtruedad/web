    ...
    default:
        ...
        if (!empty($accountId)) {
            ...
            if (!empty($characters)) {
                ...
                echo '<hr><div class="pull-right">Warehouse money: ' , (false !== $warehouseMoney) ? number_format($warehouseMoney['Money']) : '0' , '</div>';
                foreach ($characters as $character) {
                    $content .= '<div class="width100percent clear-both">
                        <h3>' . $character['Name'] . ' <small>' . character::getCharacterClassName($character['Class']) . '</small></h3>
                        <div class="width20percent pull-left">
                            <img src="views/img/' . character::getCharacterClassShortThumbnailName($character['Class']) . '.jpg" alt="Class image" class="img-thumbnail">
                        </div>
                        <div class="width70percent left-margin pull-left table-responsive">
                            <table class="table table-hover">
                            <tr><td class="width20percent"><strong>Level</strong></td><td class="width60percent">' . $character['cLevel'] . ((character::$characterMaxLevel == $character['cLevel']) ? '&nbsp;&nbsp;&nbsp;<a href="?module=account&action=reset&name=' . $character['Name'] . '" title="Reset">Click here to reset this character</a><div class="bg-primary info-box box-border top-margin">You need to have at least ' . number_format(CHARACTER_RESET_COST) . ' zen in either your inventory or warehouse to be able to reset your character.</div>' : '') . '</td></tr>
                                <tr><td><strong>Master Level</strong></td><td>' . $character['mLevel'] . '</td></tr>
                                <tr><td><strong>Reset</strong></td><td>' . $character['Reset'] . '</td></tr>
                                <tr><td><strong>Strength</strong></td><td>' . $character['Strength'] . '</td></tr>
                                <tr><td><strong>Agility</strong></td><td>' . $character['Dexterity'] . '</td></tr>
                                <tr><td><strong>Vitality</strong></td><td>' . $character['Vitality'] . '</td></tr>
                                <tr><td><strong>Energy</strong></td><td>' . $character['Energy'] . '</td></tr>';
                    if (0 < $character['Leadership']) {
                        $content .= '<tr><td><strong>Energy</strong></td><td>' . $character['Leadership'] . '</td></tr>';
                    }
                    $content .= '<tr><td><strong>Level Up Points</strong></td><td>' . $character['LevelUpPoint'] . '' . (($character['LevelUpPoint'] > 0) ? '<div class="bg-primary info-box box-border top-margin">You can use in-game commands (<em>/addstr</em>, <em>/addagi</em>, <em>/addvit</em>, <em>/addene</em>, <em>/addcmd</em>) to add available Level Up Points to the attributes.</div>' : '') . '</td></tr>
                                <tr><td><strong>Inventory Money</strong></td><td>' . number_format($character['Money']) . '</td></tr>
                            </table>
                        </div>
                    </div>';
                }
